...
const Experience: React.FC<ExperienceProps> = ({ isDark = true }) => {
  ...
  const SectionTitle = ({ title, icon: Icon, delay = 0, subtitle }: { title: string, icon: any, delay?: number, subtitle: string }) => (
    ...
    >
      ...
      <h3 className={`text-3xl md:text-5xl font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-neutral-900'}`}>
        {title}
      </h3>
      <p className={`text-xs tracking-[0.2em] font-bold uppercase ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>
        {subtitle}
      </p>
    </motion.div>
  );

  const ExperienceCard: React.FC<{ exp: any, isLeft: boolean, index: number }> = ({ exp, isLeft, index }) => {
    return (
      <div className={`relative flex flex-col md:flex-row items-center justify-between mb-32 w-full ${isLeft ? 'md:flex-row-reverse' : ''}`}>
        {/* Timeline Node */}
        ...
        >
          <div className={`group p-8 md:p-10 rounded-[3rem] border backdrop-blur-xl transition-all duration-500 ${isDark
            ? 'bg-neutral-900/40 border-white/5 hover:border-amber-500/30 shadow-2xl'
            : 'bg-white/80 border-neutral-200 hover:border-amber-500/30 shadow-xl'
            }`}>
            <div className="flex flex-wrap justify-between items-start gap-4 mb-8">
              <div>
                ...
                <h4 className={`text-3xl font-black tracking-tighter mb-2 leading-none ${isDark ? 'text-white' : 'text-neutral-900'}`}>
                  {exp.role}
                </h4>
  ...
  return (
    <section id="experience" className={`py-40 relative transition-colors duration-500 ${isDark ? 'bg-neutral-950' : 'bg-[#f8f7f3]'}`}>
      ...
      <div className="absolute top-40 right-10 pointer-events-none opacity-[0.02] select-none rotate-90 origin-right">
        <h2 className={`text-[15rem] font-black tracking-tighter ${isDark ? 'text-white' : 'text-black'}`}>MY JOURNEY</h2>
      </div>
      ...
      <div className="container mx-auto px-6 max-w-6xl relative" ref={containerRef}>
        ...
        <div className="max-w-4xl mx-auto mb-40">
          ...
          >
            ...
            <h2 className={`text-4xl md:text-7xl font-black mb-10 tracking-tight leading-[0.9] ${isDark ? 'text-white' : 'text-neutral-900'}`}>
              My <span className="text-amber-500 px-4">Professional</span> Journey<span className="text-amber-500">.</span>
            </h2>
            <p className={`text-xl md:text-2xl font-light leading-relaxed italic ${isDark ? 'text-neutral-400' : 'text-neutral-500'}`}>
            ...
            <div className="mt-12 flex gap-4">
              {[Award, Zap, Zap].map((Icon, i) => (
                <div key={i} className="w-12 h-1 bg-amber-500/20 rounded-full overflow-hidden" index={i}>
                  <motion.div
                    animate={{ x: ["-100%", "100%"] }}
